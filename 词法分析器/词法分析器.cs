...
namespace CnToCs.词法分析器
{
    ...
    public class 词法分析器
    {
        ...
        private bool 是否为注释(ref List<词法单元> 词法单元列表)
        {
            //匹配单行注释符号"/"
            if (this.当前读取位置 + 1 < this.输入文本.Length &&this.输入文本[this.当前读取位置] == '/' &&this.输入文本[this.当前读取位置 + 1] == '/')
            {
                ...
                // 提取注释内容
                string 注释内容 = 截取字符串(this.输入文本, 注释开始位置, this.当前读取位置);
                词法单元列表.Add(new 词法单元(词法单元类型.单行注释, 注释内容));
            ...
            if (this.当前读取位置 + 1 < this.输入文本.Length &&this.输入文本[this.当前读取位置] == '/' &&this.输入文本[this.当前读取位置 + 1] == '*')
            {
                ...
                // 查找结束的"*/"符号
                while (this.当前读取位置 + 1 < this.输入文本.Length && !(this.输入文本[this.当前读取位置] == '*' && this.输入文本[this.当前读取位置 + 1] == '/'))
                {
                    this.当前读取位置++;
                }
                ...
                // 提取注释内容
                string 注释内容 = 截取字符串(this.输入文本, 注释开始位置, this.当前读取位置);
                词法单元列表.Add(new 词法单元(词法单元类型.多行注释, 注释内容));
               
                // 跳过结束的"*/"符号
                if (this.当前读取位置 + 1 < this.输入文本.Length)
                {
                    this.当前读取位置 += 2;
                }
        ...
        public List<词法单元> 分词()
        {
            ...
            while (this.当前读取位置<输入文本.Length)
            {
                ...
                是否为关键词("变量",词法单元类型.变量,ref 词法单元列表)||
                ...
                if(是否为数字字面量(ref 词法单元列表)||是否为字符串字面量(ref 词法单元列表)||是否为标识符(ref 词法单元列表)||是否为注释(ref 词法单元列表))
                {
                    //跳过当前部分循环
                    continue;
                }
